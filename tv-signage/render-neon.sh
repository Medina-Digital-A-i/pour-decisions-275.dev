...
FRAMES="${FRAMES:-900}"
OUT="${OUT:-neon-out}"; mkdir -p "$OUT"
[ -d node_modules ] || npm install --silent
python3 build-neon.py
ENC="-c:v libx264 -preset medium -crf 19 -pix_fmt yuv420p -movflags +faststart"
for B in ${BOARDS:-A B}; do
  rm -rf frames && mkdir -p frames
  FILE="board-neon-$B.html" FRAMES="$FRAMES" OUT="$PWD/frames" node capture-neon.js
...
for B in A B; do
  ...
  (cd "$OUT" && ffmpeg -y -loglevel error -f concat -safe 0 -i "list-$B.txt" -c copy "Neon Menu $B - 60 min.mp4")
done
...
ffmpeg -y -loglevel error -f concat -safe 0 -i list-show.txt -c copy "Neon Menu Show (A+B) - 60 min.mp4"
